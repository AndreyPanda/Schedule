...
def test_choose_the_specialization(create_specs_and_doctors_and_clients, client):
...
def test_choose_the_doctor(create_specs_and_doctors_and_clients, client):
...
def test_choose_the_time(create_specs_and_doctors_and_clients, client):
    url = reverse("calendar") + "?doctor_slug=sidorov"
    response = client.get(url)
    assert response.status_code == 200


def test_fill_in_the_client_data(create_specs_and_doctors_and_clients, client):
    url = reverse("client_data") + "?visit_datetime=202306150800" + "?doctor=sidorov"
    response = client.get(url)
    assert response.status_code == 200


def test_user_visits_page(create_specs_and_doctors_and_clients, client):
