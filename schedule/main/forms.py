...
class AddClient(forms.ModelForm):
    ...
    def clean_last_name(self):
        ...
        if not contains_only_cyrillic(last_name):
            raise ValidationError('Фамилия может содержать только кириллицу и знак дефиса')
        return last_name
    ...
    def clean_fathers_name(self):
        ...
        if not contains_only_cyrillic(fathers_name):
            raise ValidationError('Отчество может содержать только кириллицу и знак дефиса')
        return fathers_name

    def clean_birth_date(self):
        birth_date = self.cleaned_data['birth_date']
        if not is_correct_birth_date(birth_date):
            raise ValidationError('Пожалуйста, введите дату рождения в формате "ГГГГ-ММ-ДД"')
        return birth_date

    def clean_phone(self):
        phone = self.cleaned_data['phone']
        if not is_correct_phone_number(phone):
            raise ValidationError('Номер телефона может содержать только цифры и знаки + и -')
        return phone
