            ...
            for day_index in range(len(str(doctor.working_days))):
                ...
                for k, v in months.items():
                    if k in dates_of_week[day_index].strftime("%d %B %Y"):
                        result_date = (
                            dates_of_week[day_index].strftime("%d %B %Y").replace(k, v)
                        )
                        break
...
def fill_in_the_client_data(request):
    doctor_id = Doctor.objects.get(slug=request.GET.get("doctor")).id
    if request.method == "POST":
        ...
        if form.is_valid():
            if not Visit.objects.filter(
                ...
                new_visit.save()
                URL = f"/booking_ok/?doctor={doctor_id}&visit_datetime={visit_datetime}"
                return redirect(URL)
            else:
                URL = (
                    f"/booking_nok/?doctor={doctor_id}&visit_datetime={visit_datetime}"
                )
                return redirect(URL)
    else:
        form = AddClient
